...
public class VoiceTranslationActivity extends GeneralActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Remove previous fragments (case of the app was restarted after changed
        // permission on android 6 and higher)
    ...
    protected void onStart() {
        super.onStart();
        // when we return to the app's gui based on the service that was saved in the
        // last closure we choose which fragment to start
    ...
    public void endWebRtcCall() {
        // Unbind all active service connections first to properly release resources
        for (CustomServiceConnection conn : new ArrayList<>(conversationServiceConnections)) {
            try {
                unbindService(conn);
                conn.onServiceDisconnected();
            } catch (IllegalArgumentException e) {
                // Service was not bound, safe to ignore
            }
        }
        conversationServiceConnections.clear();

        // Stop the service and navigate back to pairing screen
        stopConversationService();
        currentFragment = -1; // Reset so setFragment actually navigates
    ...
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        ...
        if (currentFragment == TRANSLATION_FRAGMENT || currentFragment == CONVERSATION_FRAGMENT) {
            /*
             * We recreate the activity only for TRANSLATION_FRAGMENT and
             * CONVERSATION_FRAGMENT because if we do not do this,
             * the keyboard sometimes not go in full screen when the screen has too little
             * space
             */
            if (config == null) {
                recreate();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
         ...
         * }
         */
        // recreate(); // was called only if the grantResults were of length 0 or were
        // neither PERMISSIONS_GRANTED nor PERMISSION_DENIED (I don't know what it is
        // for anyway)
    }
    ...
    public void startConversationService(final Notification notification,
            final Global.ResponseListener responseListener) {
        final Intent intent = new Intent(this, WebRtcVoiceTranslationService.class);
        global.getLanguage(false, new Global.GetLocaleListener() {
            ...
            public void onSuccess(CustomLocale result) {
                if (NotificationManagerCompat.from(VoiceTranslationActivity.this).areNotificationsEnabled()) {
                    ...
                    // getResources().getString(R.string.toast_missing_notification_permission),
    ...
    public void startWalkieTalkieService(final Notification notification,
            final Global.ResponseListener responseListener) {
    ...
    public synchronized void connectToConversationService(
        ...
        startConversationService(buildNotification(CONVERSATION_FRAGMENT), new Global.ResponseListener() {
            ...
            public void onSuccess() {
                CustomServiceConnection conversationServiceConnection = new CustomServiceConnection(
                        new WebRtcVoiceTranslationService.WebRtcServiceCommunicator(connectionId));
                connectionId++;
                ...
                bindService(new Intent(VoiceTranslationActivity.this, WebRtcVoiceTranslationService.class),
                        conversationServiceConnection, BIND_ABOVE_CLIENT);
            }
    ...
    public synchronized void connectToWalkieTalkieService(
            final VoiceTranslationService.VoiceTranslationServiceCallback callback,
            final ServiceCommunicatorListener responseListener) {
        ...
        startWalkieTalkieService(buildNotification(WALKIE_TALKIE_FRAGMENT), new Global.ResponseListener() {
            @Override
            public void onSuccess() {
                CustomServiceConnection walkieTalkieServiceConnection = new CustomServiceConnection(
                        new WalkieTalkieService.WalkieTalkieServiceCommunicator(connectionId));
                connectionId++;
                ...
                bindService(new Intent(VoiceTranslationActivity.this, WalkieTalkieService.class),
                        walkieTalkieServiceConnection, BIND_ABOVE_CLIENT);
            }
    ...
    public void disconnectFromConversationService(
            WebRtcVoiceTranslationService.WebRtcServiceCommunicator conversationServiceCommunicator) {
        int index = -1;
        boolean found = false;
        for (int i = 0; i < conversationServiceConnections.size() && !found; i++) {
            if (conversationServiceConnections.get(i).getServiceCommunicator()
                    .equals(conversationServiceCommunicator)) {
                index = i;
                found = true;
            }
        }
    ...
    public void disconnectFromWalkieTalkieService(
            WalkieTalkieService.WalkieTalkieServiceCommunicator walkieTalkieServiceCommunicator) {
        int index = -1;
        boolean found = false;
        for (int i = 0; i < walkieTalkieServiceConnections.size() && !found; i++) {
            if (walkieTalkieServiceConnections.get(i).getServiceCommunicator()
                    .equals(walkieTalkieServiceCommunicator)) {
                index = i;
                found = true;
            }
        }
    ...
    public void stopConversationService() {
        stopService(new Intent(this, WebRtcVoiceTranslationService.class));
    }
    ...
    public void addCallback(Callback callback) {
        // in this way the listener will listen to both this activity and the
        // communicator
        if (global.getBluetoothCommunicator() != null) {
